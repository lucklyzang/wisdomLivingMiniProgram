...
var _breathe = _interopRequireDefault(__webpack_require__(/*! ./healthMonitoringPackage/pages/breathe/breathe.vue */ 198));
// @ts-ignore
...
/* harmony import */ var _breathe_vue_vue_type_template_id_3b9bb925___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./breathe.vue?vue&type=template&id=3b9bb925& */ 199);
/* harmony import */ var _breathe_vue_vue_type_script_lang_js___WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./breathe.vue?vue&type=script&lang=js& */ 201);
...
/* harmony import */ var _breathe_vue_vue_type_style_index_0_lang_scss___WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./breathe.vue?vue&type=style&index=0&lang=scss& */ 203);
      ...
      (0, _device.sleepStatisticsDetails)(data).then(function (res) {
        if (res && res.data.code == 0) {
          ...
          if (JSON.stringify(res.data.data) == '{}' || questData.breath.timeList.length == 0) {
            ...
            _this3.initDayText = Math.floor(questData.breath.timeList[0]['value']);
            _this3.initDayTime = _this3.getNowFormatDate(new Date(questData.breath.timeList[0]['time']), 1);
            _this3.lowest = Math.floor(questData.breath.lowest);
            _this3.highest = Math.floor(questData.breath.highest);
            _this3.average = Math.floor(questData.breath.average);
          ...
          if (type == 'week') {
            ...
            if (JSON.stringify(res.data.data) == '{}' || questData.respVOList.length == 0) {
              ...
              if (questData.respVOList[0]['startTime'] == _this4.currentStartWeekDate) {
                _this4.initWeekText = "".concat(Math.floor(questData.respVOList[0]['breathMinValue']), "-").concat(Math.floor(questData.respVOList[0]['breathMaxValue']));
                _this4.lowest = Math.floor(questData.respVOList[0]['breathMinValue']);
                _this4.highest = Math.floor(questData.respVOList[0]['breathMaxValue']);
                _this4.average = Math.floor(questData.respVOList[0]['breathAverage']);
              } else {
                _this4.initWeekText = '-';
                _this4.lowest = '-';
                _this4.highest = '-';
            ...
            if (JSON.stringify(res.data.data) == '{}' || _questData.respVOList.length == 0) {
              ...
              if (_questData.respVOList[0]['startTime'] == "".concat(_this4.currentMonthDate, "-01")) {
                _this4.initMonthText = "".concat(Math.floor(_questData.respVOList[0]['breathMinValue']), "-").concat(Math.floor(_questData.respVOList[0]['breathMaxValue']));
                _this4.lowest = Math.floor(_questData.respVOList[0]['breathMinValue']);
                _this4.highest = Math.floor(_questData.respVOList[0]['breathMaxValue']);
                _this4.average = Math.floor(_questData.respVOList[0]['breathAverage']);
              } else {
                _this4.initMonthText = '-';
                _this4.lowest = '-';
                _this4.highest = '-';
